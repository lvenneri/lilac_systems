...
def read_one_sample():
    """
    Grabs and returns all the voltage sample from the specified channel.
    """
    # We assume that the necessary MCC DAQ libraries and objects
    # are already imported or available in the environment.
    address = select_hat_device(HatIDs.MCC_128)
    hat = mcc128(address)
    ...
    readings = []
    for ch in range(4):
        readings.append(hat.a_in_read(ch, OptionFlags.DEFAULT))
